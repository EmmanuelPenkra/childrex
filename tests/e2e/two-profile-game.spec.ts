...
test('two isolated Chrome profiles complete a synchronized room journey',async({browser},testInfo)=>{
  ...
  try{
    ...
    await expect(host.getByRole('heading',{name:'Gather your teams'})).toBeVisible();
    ...
    await expect(host.locator('.player-copy small')).toContainText('Amara');

    await guest.goto(`/sequence/join?code=${code}`);
    await expect(guest.getByRole('heading',{name:'Enter the two-digit code'})).toBeVisible();
    await expect(guest.getByLabel('Game code')).toHaveValue(code);
    await guest.getByRole('button',{name:/Join game/}).click();
    await expect(guest.getByRole('heading',{name:'Gather your teams'})).toBeVisible();
    ...
    await host.getByRole('button',{name:'Menu'}).click();await expect(host.getByRole('heading',{name:'Paused for you'})).toBeVisible();
    await host.getByRole('button',{name:/Sound/}).click();await expect(host.getByRole('button',{name:/Sound/})).toContainText('On');
    await shot(host,testInfo,'09-menu-overlay');await host.getByRole('button',{name:'Back to game'}).click();
    await expect(host.getByRole('heading',{name:'Paused for you'})).toBeHidden();
    ...
    await expect(host.getByText(/TEAM WINS/)).toBeVisible();await expect(guest.getByText(/TEAM WINS/)).toBeVisible();
    ...
    await expect(host.getByText(/TEAM WINS/)).toBeHidden();await expect(guest.getByText(/TEAM WINS/)).toBeHidden();
